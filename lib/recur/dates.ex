defmodule Dates do
  ...
  def day_of_week(date, week_start) do
    ...
    base_dow = Enum.find_index(@week_days, &(&1 == week_start)) + 1
    rem(dow - base_dow, 7) + 1
  end

  def day_of_week_name(day_number, week_start) do
    base_dow = Enum.find_index(@week_days, &(&1 == week_start))
    offset_dow = rem(base_dow + day_number, 7) - 1
    Enum.at(@week_days, offset_dow)
  end
